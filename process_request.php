...
function automaticallyWatermarkAndUpdate($document_url, $company_name, $client_id) {
    try {
        // Create main permits directory if it doesn't exist - FIXED PATH
        $permitsDir = "documents/permits/";
        if (!is_dir($permitsDir)) {
            mkdir($permitsDir, 0777, true);
        }
        
        // Get file info from the original document URL
        ...
        // AUTOMATIC: For all other file types, create approved version
